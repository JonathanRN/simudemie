...
public abstract class GestionnaireOnglet<T extends Serializable> {
    ...
    protected void ajouter(T object) {list.add(object); sauvegarder();}

    public void sauvegarder() {fileHelper.sauvegarder(list);}
    
    public void charger() {list = fileHelper.charger();}
    
    public void supprimer(int index) {list.remove(index); sauvegarder();}
